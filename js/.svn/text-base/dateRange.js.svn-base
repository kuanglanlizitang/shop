 ...
 */
function pickerDateRange(inputId, options, callback) {
	/**
	 * 默认配置参数数据，每个参数涵义在后解释
	 */
	var defaults = {
	...
	// 清除
	this.clearBtn = 'clearBtn_' + suffix;
	...
	// 初始化日期选择器面板的HTML代码串
	var btns;
	if(this.mOpts.language == 'cn') {
		btns = ['确定', '取消', '清除'];

		if(false == this.mOpts.isSingleDay) {
			this.mOpts.firstCss = 'first'; //起始样式
			this.mOpts.lastCss = 'last'; //结束样式
			this.mOpts.clickCss = 'first'; //点击样式
		} else {
			this.mOpts.firstCss = 'today';
			this.mOpts.clickCss = 'today'; //点击样式
		}

	} else {
		btns = ['Submit', 'Cancel', 'Clear'];

		if(false == this.mOpts.isSingleDay) {
			this.mOpts.firstCss = 'first-en'; //起始样式
			this.mOpts.lastCss = 'last-en'; //结束样式
			this.mOpts.clickCss = 'first-en'; //点击样式
		} else {
			this.mOpts.firstCss = 'today-en';
			this.mOpts.clickCss = 'today-en'; //点击样式
		}
	}
	var wrapper = {
		gri: [
			...
			'</div>',
			'<div>',
			'<input type="button" name="' + this.submitBtn + '" id="' + this.submitBtn + '" value="' + btns[0] + '" />',
			'&nbsp;<input class="ta_btn"  type="button" id="' + this.clearBtn + '" value="' + btns[2] + '"/>',
			'&nbsp;<a id="' + this.closeBtn + '" href="javascript:;">' + btns[1] + '</a>',
			'</div>',
			'</div>',
			'</div>'
		],
		ta: [
			...
			'</div>',
			'<div class="frm_btn">',
			'<input class="ta_btn ta_btn_primary" type="button" name="' + this.submitBtn + '" id="' + this.submitBtn + '" value="' + btns[0] + '" />',
			'<input class="ta_btn" type="button" id="' + this.clearBtn + '" value="' + btns[2] + '"/>',
			'<input class="ta_btn" type="button" id="' + this.closeBtn + '" value="' + btns[1] + '"/>',
			'</div>',
			'</div>',
			'</div>'
		]
	...
	$('#' + this.submitBtn).bind('click', function() {
		...
		if(__method.hasclear) {
			console.log("2222");
			$('#' + __method.startDateId).val('');

			if($('#' + __method.endDateId) != undefined) {
				$('#' + __method.endDateId).val('');
			}
			__method.hasclear = false;
		} else {
		...
		document.getElementById(__method.inputId).setAttribute("oldstartDefDate", $('#' + __method.startDateId).val());
		document.getElementById(__method.inputId).setAttribute("oldendDefDate", $('#' + __method.endDateId).val());
	...
	$('#' + this.closeBtn).bind('click', function() {
		...
		$('#' + __method.startDateId).val(document.getElementById(__method.inputId).getAttribute("oldstartDefDate"));
		$('#' + __method.endDateId).val(document.getElementById(__method.inputId).getAttribute("oldendDefDate"));
...
pickerDateRange.prototype.show = function(isCompare, __method) {
	...
	$("#" + this.calendarId).css('top', pos.top + 40 + 'px');
	//第一次显示的时候，一定要初始化输入框
...
pickerDateRange.prototype.close = function(btnSubmit) {
	...
	if(btnSubmit) {
		...
		if($('#' + this.inputId).val() != '' && $('#' + this.inputId).val().length < 15) {
			document.getElementById(this.inputId).innerText = "";
			if(this.mOpts.language == 'cn') {
				navigator.notification.confirm('请选择结束日期', function() {}, getStringTips(), getStringOk());
			} else {
				navigator.notification.confirm('Please choose the end date', function() {}, getStringTips(), getStringOk());
			}
			return;
		}
		...
		document.getElementById(this.inputId).setAttribute("name", $('#' + this.inputId).val());
		document.getElementById(this.inputId).innerText = "";
		$("#" + __method.calendarId).css('display', 'none');
		console.log("normal");
		this.callback(document.getElementById(this.inputId).getAttribute("name"));
	} else {
		$("#" + __method.calendarId).css('display', 'none');
		console.log("cancel");
		if(__method.hasclear){
	...
	if(isTaTheme) {
		...
		if(this.mOpts.magicSelect) {
			...
			if(this.mOpts.language == 'cn') {
				...
				$(cap).append(this.translate2En(year + '年' + (month + 1) + '月'));
			}
		}
		...
		if(this.mOpts.language == 'cn') {
			...
			$(td).append(this.translate2En(year + '年' + (month + 1) + '月'));
		}
...
/**
 * @description 把中文年月转英文年月 2017年8月  Aug 2017
 * @param {String} str 时间字符串
 */
pickerDateRange.prototype.translate2En = function(str) {
	var strs = str.split('年');
	var month = strs[1].replace('月', '');
